...
contract Whitelist is Ownable {
    ...
    bool private whitelistEnabled = false;
...
contract Blacklist is Ownable {
    ...
    // flag controlling whether whitelist is enabled.
    bool private blacklistEnabled = false;

    event AddToBlacklist(address indexed _newAddress);
    event RemoveFromBlacklist(address indexed _removedAddress);

    /**
   * @dev Enable or disable the whitelist
   * @param _enabled bool of whether to enable the whitelist.
   */
    function enableBlacklist(bool _enabled) public onlyOwner {
   ...
   * @param _newAddress address to be added to the whitelist
   */
    function addToBlacklist(address _newAddress) public onlyOwner {
        _blacklist(_newAddress);
        emit AddToBlacklist(_newAddress);
    }
   ...
   * @param _removedAddress address to be removed from the whitelist
   */
    function removeFromBlacklist(address _removedAddress) public onlyOwner {
        _unBlacklist(_removedAddress);
        emit RemoveFromBlacklist(_removedAddress);
    }
   ...
   * @param _removedAddress address to unwhitelisted
   */
    function _unBlacklist(address _removedAddress) internal {
        blacklistMap[_removedAddress] = false;
    }
   ...
   * @param _newAddress address to be added to the whitelist
   */
    function _blacklist(address _newAddress) internal {
        blacklistMap[_newAddress] = true;
    }
}
    ...
    Blacklist
{
    ...
    function initBlacklist(address[] memory _blacklistees) public onlyOwner {
        for (uint256 i = 0; i < _blacklistees.length; i++) {
            address creator = _blacklistees[i];
    ...
    function addNewToken(string memory _uri) public {
        ...
        require(isBlacklisted(msg.sender) == false, "must be whitelisted to create tokens");
